...
from offlinerlkit.policy import AWRPolicy, MemAWRPolicy
...
def train(args=get_args()):
    ...
    critic1_backbone = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=[256, 256]) # V(s)
    critic2_backbone = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=[256, 256]) # V(s)
    ...
    log_dirs = make_log_dirs_td3bc(task_name=args.task, chosen_percentage=args.chosen_percentage, algo_name=args.algo_name, seed=args.seed, args=vars(args), record_params=record_params, root_dir = "algos/exp_awr")
