...
def train(args=get_args()):
    ...
    # load policy weights
    fol = logger.checkpoint_dir if 'carla' in args.task else logger.model_dir
    policy.load_state_dict(torch.load(os.path.join(fol, "policy.pth")))
    ...
    # plot of instantaneous reward vs dist from memories
    nodes_obs = torch.as_tensor(dataset["memories_obs"], device=args.device, dtype=torch.float32)
    all_rewards, all_dists, norm_score = policy_trainer.get_all_reward_and_dists_from_memories(nodes_obs)
    print(f'Mean Normalized score for the {args.eval_episodes} eval_episodes is {norm_score}.')
    os.makedirs("viz/reward_vs_dists", exist_ok=True)
    np.save(f"viz/reward_vs_dists/{args.task}_{args.algo_name}_frac{args.num_memories_frac}_lamda{args.lamda}_Lipz{args.Lipz}_rewards.npy", all_rewards)
    np.save(f"viz/reward_vs_dists/{args.task}_{args.algo_name}_frac{args.num_memories_frac}_lamda{args.lamda}_Lipz{args.Lipz}_dists.npy", all_dists)
    # plt.scatter(all_dists, all_rewards)
    # plt.xlabel("Distance from expert distribution (via memories)")
    # plt.ylabel("Instantaneous reward")
    # plt.savefig("viz/reward_vs_dists/reward_vs_dist.png")
